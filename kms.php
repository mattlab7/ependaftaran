...
if (isset($_POST['btn-signup'])){
	if (is_numeric($_POST['id'])) /* put hidden one in form first, check website */{
		...
		if ($stname == '' || $stkp == '' || $stclass == '' || $staddress == '' || $stparent == '' || $prname == '' || $prcontact == ''){
			...
			mysql_query("UPDATE datamurid SET namaMurid='$stname', kpMurid='$stkp', kelas='$stclass', alamat='$staddress', penjagaMurid='$stparent', namaPenjaga='$prname', telefonPenjaga='$prcontact' WHERE idMurid='$id'") or die(mysql_error());
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0){
		$id = $_GET['id'];
		$result = mysql_query("SELECT * FROM datamurid WHERE idMurid=$id") or die(mysql_error());
